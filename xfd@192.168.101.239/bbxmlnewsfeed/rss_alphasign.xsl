...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/Projects/Project[@name='autotrader-branch :: build']|/Projects/Project[@name='autotrader-trunk :: build']|/Projects/Project[@name='autotrader-branch :: Stub-Twist-Tests']">
...
-->
		...
		<!-- <xsl:if test="contains(type,'A') or contains(type,'B')"> -->
			...
			<xsl:otherwise>
				...
				<xsl:text>Build: </xsl:text>
				<xsl:value-of select="@lastBuildLabel"/>
				<xsl:text> was sucessfully built.</xsl:text>
			</xsl:otherwise>
		</xsl:choose>
		<!-- <xsl:text> @ </xsl:text>
