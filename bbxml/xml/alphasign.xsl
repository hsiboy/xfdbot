...
<xsl:stylesheet
 ...
 <!-- XDF
 Note that for 12 hour time displays, any leading zero will be suppressed on the
 hours, whereas this will not be the case for 24 hour displays. Time displays 
 are subject to similar word fit rules as standard text, though there will 
 always be a break between them and other text, even if there are no 
 intervening spaces. 
 -->
  ...
  <xsl:template match="timeFormat">
    <xsl:text>E'</xsl:text>
    <xsl:choose>
  <!-- “M” (4DH) 24 hour format	20:56 -->
      <xsl:when test="@format = '24-hour'">
	<xsl:text>M</xsl:text>
      </xsl:when>
  <!-- “N” (4EH)		24 hour format, blinking colon	20:56 -->
      <xsl:when test="@format = '24-hour-blinking-colon'">
	<xsl:text>N</xsl:text>
      </xsl:when>
  <!-- “P” (50H)		12 hour format	8:56 -->
      <xsl:when test="@format = '12-hour'">
	<xsl:text>P</xsl:text>
      </xsl:when>
  <!-- “Q” (51H)		12 hour format, blinking colon	8:56 -->
      <xsl:when test="@format = '12-hour-blinking-colon'">
	<xsl:text>Q</xsl:text>
      </xsl:when>
  <!-- “S” (53H)		12 hour format, AM/PM 	8:56 PM -->
      <xsl:when test="@format = '12-hour-am-pm'">
	<xsl:text>S</xsl:text>
      </xsl:when>
  <!-- “T” (54H)		12 hour, AM/PM, blinking colon	8:56 PM -->
      <xsl:when test="@format = '12-hour-am-pm-blinking-colon'">
	<xsl:text>T</xsl:text>
      </xsl:when>
  <!-- “m” (6DH)		24 hour + seconds	20:56.34 -->
      <xsl:when test="@format = '24-hour-seconds'">
	<xsl:text>m</xsl:text>
      </xsl:when>
  <!-- “n” (6EH)		24 hour + seconds, blinking colon	20:56.34 -->      
      <xsl:when test="@format = '24-hour-seconds-blinking-colon'">
	<xsl:text>n</xsl:text>
      </xsl:when>
  <!-- “p” (70H)		12 hour + seconds	8:56.34 -->      
      <xsl:when test="@format = '12-hour-seconds'">
	<xsl:text>p</xsl:text>
      </xsl:when>
  <!-- “q” (71H)		12 hour + seconds, blinking colon	8:56.34 -->
      <xsl:when test="@format = '12-hour-seconds-blinking-colon'">
	<xsl:text>q</xsl:text>
      </xsl:when>
  <!-- “s” (73H)		12 hour + seconds, AM/PM 8:56.34 PM -->      
      <xsl:when test="@format = '12-hour-seconds-am-pm'">
	<xsl:text>s</xsl:text>
      </xsl:when>
  <!-- “t” (74H)		12 hour + secs, AM/PM, blinking colon	8:56.34 PM -->      
      <xsl:when test="@format = '12-hour-seconds-am-pm-blinking-colon'">
	<xsl:text>t</xsl:text>
      </xsl:when>
  <!-- “x” (78H)		Seconds-only display (2 digits)	34 -->
      <xsl:when test="@format = 'seconds'">
	<xsl:text>x</xsl:text>
      </xsl:when>      
  <!-- “y” (79H)		Sub-seconds display (100ms/10ms)	68 -->
      <xsl:when test="@format = 'sub-seconds'">
	<xsl:text>y</xsl:text>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
...
<!-- used to set day of week -->
  ...
  <xsl:template match="calendarDate">
   ...
   <xsl:template match="timeoutMessage">
    <xsl:text>T</xsl:text>
    <xsl:call-template name="toPaddedHex">
	  <xsl:with-param name="dec" select="@timeout"/>
...
-->
  ...
  <xsl:template match="date[@format = 'day']">
   ...
   <!-- XFD only from here -->
  <xsl:template match="date[@format = 'DD MMM YYYY']">
    <xsl:text>_0BA</xsl:text>
   </xsl:template>
  <xsl:template match="date[@format = 'MM/DD/YYYY']">
    <xsl:text>_0BB</xsl:text>
   </xsl:template>
  <xsl:template match="date[@format = 'DD/MM/YYYY']">
    <xsl:text>_0BC</xsl:text>
   </xsl:template>
  <xsl:template match="date[@format = 'MM-DD-YYYY']">
    <xsl:text>_0BD</xsl:text>
   </xsl:template>
  <xsl:template match="date[@format = 'DD-MM-YYYY']">
    <xsl:text>_0BE</xsl:text>
   </xsl:template>
  <xsl:template match="date[@format = 'MM.DD.YYYY']">
    <xsl:text>_0BF</xsl:text>
   </xsl:template>
  <xsl:template match="date[@format = 'DD.MM.YYYY']">
    <xsl:text>_0BG</xsl:text>
   </xsl:template>
  <xsl:template match="date[@format = 'MM DD YYYY']">
    <xsl:text>_0BH</xsl:text>
   </xsl:template>
  <xsl:template match="date[@format = 'DD MM YYYY']">
    <xsl:text>_0BI</xsl:text>
   </xsl:template>
  ...
  <!-- alpha Effect Name -->
    ...
    <!-- XFD basic effect names -->
    <a:mode name="slowRotate" value="a"/>
    <a:mode name="hold" value="b"/>
    <a:mode name="flash" value="c"/>
    <a:mode name="auto" value="d"/>
    <a:mode name="scrollUp" value="e"/>
    <a:mode name="scrollDown" value="f"/>
    <a:mode name="fastScrollLeft" value="g"/>
    <a:mode name="fastScrollRight" value="h"/>
    <a:mode name="wipeUp" value="i"/>
    <a:mode name="wipeDown" value="j"/>
    <a:mode name="wipeLeft" value="k"/>
    <a:mode name="wipeRight" value="l"/>
    <a:mode name="fullScreenScrollUp" value="m"/>
    <a:mode name="auto" value="o"/>
    <a:mode name="implodeScroll" value="p"/>
    <a:mode name="explodeScroll" value="q"/>
    <a:mode name="implodeWipe" value="r"/>
    <a:mode name="explodeWipe" value="s"/>
    <a:mode name="slowScrollLeft" value="t"/>
    <a:mode name="vertical-explode-scroll" value="u"/>
    <a:mode name="slowScrollRight" value="v"/>
    <a:mode name="slowScrollUp" value="w"/>
    <a:mode name="slowScrollDown" value="x"/>
    <!-- XFD Extended Effects Modes -->
    <a:mode name="twinkle" value="n0"/>
    <a:mode name="dissolve" value="n1"/>
    <a:mode name="snow" value="n2"/>
    <a:mode name="interlockWipe1" value="n3"/>
    <a:mode name="fastSwitch" value="n4"/>
    <a:mode name="slide" value="n5"/>
    <a:mode name="dissolveWipe" value="n6"/>
    <a:mode name="newCursorWipe" value="n7"/>
    <a:mode name="interlockWipe2" value="n8"/>
    <a:mode name="interlockScroll1" value="n9"/>
    <a:mode name="interlockScroll2" value="nA"/>
    <a:mode name="slowDropDown" value="nB"/>
    <a:mode name="colourCycle" value="nC"/>
    <a:mode name="slowFadeEffect" value="nD"/>
    <a:mode name="fastFadeEffect" value="nE"/>
    <a:mode name="fastDropDown" value="nF"/>
    <a:mode name="colourSplitScroll1" value="nG"/>
    <a:mode name="colourSplitScroll2" value="nH"/>
    <a:mode name="biDirColourSplitScroll1" value="nI"/>
    <a:mode name="biDirColourSplitScroll2" value="nJ"/>
    <a:mode name="unidirColourSplitScroll1" value="nK"/>
    <a:mode name="unidirColourSplitScroll2" value="nL"/>
    <a:mode name="coverRightLeft" value="nM"/>
    <a:mode name="coverLeftRight" value="nN"/>
    <a:mode name="revealRightLeft" value="nO"/>
    <a:mode name="revealLeftRight" value="nP"/>
    <a:mode name="slowSwitch" value="nS"/>
    <a:mode name="verticalImplodeScroll" value="nU"/>
    <a:mode name="verticalExplodeScroll" value="nV"/>
    <a:mode name="verticalImplodeWipe" value="nW"/>
    <a:mode name="verticalExplodeWipe" value="nX"/>
    <a:mode name="simultaneousColourSplitScroll1" value="nY"/>
    <a:mode name="biDirSeqColourSplitScroll1" value="nZ"/>
  </a:modeLookup>
